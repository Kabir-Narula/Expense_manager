...
export default function ManageAccount() {
  ...
  const deleteAccount = async () => {
    if (!isOwner) return;
    if (!confirm("This will delete the shared account for all members. Continue?")) return;
    ...
    <div className="md:ml-72 md:pt-8 pt-20 p-8 min-h-screen bg-gray-50">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500">{currentAccount?.name || (currentAccount?.type === 'personal' ? 'Personal' : 'Shared')}</p>
        </div>
        {isOwner && currentAccount?.type === 'shared' && (
          <button onClick={deleteAccount} className="bg-red-600 text-white px-4 py-2 rounded-md">Delete Shared Account</button>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm p-4">
          ...
          <table className="w-full">
            ...
            <tbody>
              {members.map((m) => (
                <tr key={m.userId} className="border-b last:border-b-0 hover:bg-gray-50">
                  <td className="py-3">
