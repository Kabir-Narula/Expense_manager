...
export const exportIncomeToCSV = (incomeData, year, memberFilter = 'all') => {
  try {
    ...
    Object.entries(incomeData).forEach(([month, data]) => {
      data.income.forEach(item => {
        flatData.push({
          ...
          'Created By': item.createdBy?.fullName || item.createdBy?.email || 'You',
          Icon: item.icon || '',
    ...
    link.setAttribute('download', `income_${year}${filterSuffix}_${timestamp}.csv`);
...
export const exportExpenseToCSV = (expenseData, year, memberFilter = 'all') => {
  try {
    ...
    Object.entries(expenseData).forEach(([month, data]) => {
      data.income.forEach(item => {
        flatData.push({
          Date: formatDate(item.date),
          Type: 'Expense',
    ...
    const filterSuffix = memberFilter !== 'all' ? `_${memberFilter}` : '';
    link.setAttribute('href', url);
    link.setAttribute('download', `expenses_${year}${filterSuffix}_${timestamp}.csv`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
...
export const exportIncomeToPDF = (incomeData, year, memberFilter = 'all') => {
  try {
    ...
    Object.entries(incomeData).forEach(([month, data]) => {
      if (data && data.income && Array.isArray(data.income)) {
        data.income.forEach(item => {
...
export const exportExpenseToPDF = (expenseData, year, memberFilter = 'all') => {
  try {
    ...
    doc.setFont(undefined, 'normal');
    doc.setFontSize(10);
    doc.text(`Total Expenses: $${formatCurrency(totalExpense)}`, 14, finalY + 23);
    doc.text(`Number of Transactions: ${flatData.length}`, 14, finalY + 30);
